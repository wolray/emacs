...
(defmacro m-map-key (obj key)
  `(let* ((ks (cadr ',key)) (mk (kbd (concat "M-g " ks))) (vs (substring ks -1)))
...
(defmacro m-number-key (key cmd)
  `(progn
     (define-key minibuffer-local-map ,key 'self-insert-command)
...
(dolist (s (mapcar 'char-to-string (number-sequence ?a ?z)))
  (let* ((ks (concat "C-M-" s)) (mks (concat "M-g " ks)))
    (define-key key-translation-map (kbd ks) (kbd mks))
    (global-unset-key (kbd mks))))

(global-set-key (kbd "C-c C-c") 'eval-buffer)
(global-set-key (kbd "C-r") 'c-other-window)

(m-map-key 'backward-kill-paragraph (kbd "C-n"))
(m-map-key 'backward-kill-paragraph (kbd "M-n"))
...
(m-map-key 'c-highlight-symbol-definition (kbd "M-y"))
(m-map-key 'c-highlight-symbol-next (kbd "M-o"))
(m-map-key 'c-highlight-symbol-prev (kbd "M-u"))
(m-map-key 'c-highlight-symbol-recall (kbd "M-h"))
...
(m-map-key 'c-other-window (kbd "M-r"))
...
(m-map-key 'c-word-capitalize (kbd "M-j"))
(m-map-key 'c-word-downcase (kbd "M-k"))
(m-map-key 'c-word-upcase (kbd "M-l"))
