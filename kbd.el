  ...
  (let ((pfx (substring (symbol-name func) -1)))
    `(progn
	 ...
	 (let ((k (read-key-sequence nil)) ks cmd)
	   ...
	   (setq ks (concat "C-" ,pfx " " (when (string= ,pfx k) "C-") k)
...
(define-key visual-mode-map (kbd "n") 'c-pt-set-mark)
...
(global-set-key (kbd "H-i") 'c-hs-highlight)
...
(global-set-key (kbd "H-n") 'c-pt-exchange-mark)
