...
(let ((map visual-mode-map))
  ...
  (define-key map (kbd "4") 'split-line)
  ...
  (define-key map (kbd "SPC 6") 'c-switch-to-scratch)
  ...
  (define-key map (kbd "SPC a") 'mark-whole-buffer)
  ...
  (define-key map (kbd "SPC q ") 'c-open-folder)
  ...
  (define-key map (kbd "v") 'c-toggle-comment)
  (define-key map (kbd "w") 'c-kill-ring-save)
  (define-key map (kbd "x") map)
  (define-key map (kbd "y") 'C-y)
  (define-key map (kbd "z") map)
  )
...
(with-eval-after-load 'bs
  (let ((map bs-mode-map))
    ...
    (define-key map (kbd "C") map)
    (define-key map (kbd "M") map)
    (define-key map (kbd "S") map)
    (define-key map (kbd "[") 'bs-up)
    (define-key map (kbd "]") 'bs-down)
    (define-key map (kbd "b") map)
    (define-key map (kbd "f") map)
    (define-key map (kbd "i") 'bs-select)
    (define-key map (kbd "k") map)
    (define-key map (kbd "n") map)
    (define-key map (kbd "o") 'bs-down)
    (define-key map (kbd "p") map)
    (define-key map (kbd "r") 'bs-select-other-window)
    (define-key map (kbd "t") map)
...
(with-eval-after-load 'matlab
  (let ((map matlab-mode-map))
    (define-key map (kbd "C-c C-q") map)
    ))
